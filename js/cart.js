...
            totalCost(response.price);
            totalPrix(produit, response.price);

let totalPrice = document.getElementById("totalPrice");
       totalPrice.innerHTML = totalPrix(response);
         
       
        }
    })
...
// totalQuantity.innerHTML = totalCost();
//Calcul de la quantité totale des articles sélectionés
function  totalCost (produit, price) {
    let cart = JSON.parse(localStorage.getItem("produit"));
    let totalQuantity = document.getElementById("totalQuantity")
    let total = cart.reduce((tota, item) => tota +=  item.pQuantity, 0)
...
function totalPrix(produit , price) {
    let cart = JSON.parse(localStorage.getItem("produit"));
    let totaux = document.getElementById("totalPrice")
    
        let tota= cart.reduce((total, item) => total + produit.price ++ * item.pQuantity, 0)
    totaux.innerHTML = tota
    return tota
    console.log(tota);
